...
- 🟡 **확장 비전 (v3) — "총무 N빵 포털" (랜딩 착수)**: 골프뿐 아니라 모든 모임 총무 N빵 도구로 확장(테니스·배드민턴·산악회·해외여행·회식·동창회·캠핑·낚시·자전거·경조사…). **한 엔진 위 테마 버티컬**(trip에 `category` 추가 + 프리셋), 포털 랜딩, 공유 바이럴+SEO로 트래픽→광고. 골프는 첫 간판 버티컬. 상세 = `기획안.md` 10장.
  - ✅ **포털 메인 랜딩 완료**: `portal/index.html`(repo) → **deoklabs.xyz 루트**(`/opt/bitnami/apache/htdocs/index.html`)로 배포. "총무N빵" 브랜드, 히어로·서비스 12종 카드(골프총무만 `/golfChongmu/` 링크, 나머지 "준비중")·왜 총무N빵·인기 골프여행지 가이드·커뮤니티 티저·회원 CTA·FAQ·푸터(개인정보/사용법 링크). Tailwind+Pretendard, 에메랄드 테마. AdSense 스니펫·ads.txt 포함 → **콘텐츠 풍부한 루트로 AdSense 심사 대비**. 기존 텐트깔끄미 업무포털 루트는 `index.html.bak.20260610_portal`로 백업(서브앱 /discount·/flight·/CS는 직접 URL로 계속 동작, 링크만 미노출).
  - ⬜ 다음: 서브 버티컬 실제 구현(테니스총무 등 = golfChongmu 엔진 재사용 + category), 커뮤니티/게시판, 포털 자체 회원/소개 페이지.
- 🟡 **AdSense (코드 적용 완료, 사이트 검토 대기)**: 같은 구글 계정(deok22@gmail.com, 이미 tentme.co.kr 승인됨)의 발행자 ID **`pub-6552756009906366`** 사용. AdSense는 **도메인 단위**라 서브경로(`/golfChongmu`)가 아니라 **도메인 `deoklabs.xyz`를 사이트로 등록**(승인되면 그 도메인 모든 서브경로에 광고 가능). 적용한 것: ① `frontend/index.html`·`share.html` `<head>`에 adsbygoogle.js 스니펫(client=ca-pub-6552756009906366) → 라이브 반영. ② **`/opt/bitnami/apache/htdocs/ads.txt`**(=deoklabs.xyz 루트 docroot)에 `google.com, pub-6552756009906366, DIRECT, f08c47fec0942fa0` 생성 → `https://deoklabs.xyz/ads.txt`(bare→www 301도 OK) 라이브 확인. ③ **루트 홈페이지**(`/opt/bitnami/apache/htdocs/index.html` = 텐트깔끄미 포털)에도 같은 스니펫 추가(소유권 확인 크롤러가 도메인 루트를 보기 때문). 원본 백업=`index.html.bak.adsense`. ④ AdSense "콘텐츠 가치 부족" 거절 대비 **공개 콘텐츠 페이지 추가**: `frontend/guide.html`(소개·사용법·FAQ), `frontend/privacy.html`(개인정보처리방침 — AdSense 쿠키/광고 고지 포함). 로그인 화면(`index.html` view-auth)·`share.html` 푸터에서 링크. **남은 것(사용자 클릭)**: AdSense 대시보드 → 사이트 → `deoklabs.xyz` 추가 → (코드/ads.txt 방식으로) 소유권 확인 → 콘텐츠 검토 대기 → 승인되면 광고단위를 `#ad-slot`에 삽입(또는 자동광고 ON).
- ✅ **PWA (완료) — 홈 화면 설치 가능 앱**: 웹 코드 그대로 "설치 없는 웹앱"을 한 단계 올려 **홈 화면 아이콘 + 전체화면(standalone) 실행**. 추가 파일: `frontend/manifest.json`(이름·아이콘·테마색 #059669·standalone, **경로 전부 상대경로**라 루트/서브경로 양립), `frontend/sw.js`(service worker), `frontend/icons/*.png`(192·512·maskable·apple-touch-180·favicon-32 = ⛳깃발+공+홀, 에메랄드. 생성기 `tools/gen-icons.py` Pillow로 도형 직접 드로잉 → 이모지 폰트 의존 없음), `index.html` `<head>`에 manifest·iOS 메타·아이콘 링크 + 하단에 SW 등록(`navigator.serviceWorker.register("sw.js")`). **SW 캐싱 원칙(중요)**: 라이브+scp 재배포라 "옛 화면 박제" 방지 최우선 → `/api/`·인증·외부(환율 open.er-api·AdSense·Tailwind/Pretendard CDN)는 **절대 미간섭(통과)**, 앱 셸(HTML/JS/CSS)은 **network-first**(항상 최신, 오프라인만 캐시 폴백), 아이콘만 cache-first, 캐시 버전(`gd-v1`) 올리면 활성화 시 옛 캐시 자동 삭제. **배포**: 프론트 파일만 scp(`manifest.json`·`sw.js`·`icons/`·`index.html`) → `/home/bitnami/golfChongmu/frontend/`, **pm2 재시작 불필요**. HTTPS 적용돼 있어 실기기 "홈 화면에 추가" 즉시 동작. share.html(공개 공유페이지)은 PWA 대상 제외(앱 본체는 index.html). **다음(원하면)**: 앱스토어 출시·iOS 푸시가 필요해지면 Capacitor로 WebView 래핑(이 웹코드 95% 재사용).
- ⬜ **Phase 3 (다음)**: ① ~~AdSense 신청~~(코드 적용됨, 검토 대기) ② JSON DB→실DB ③ 비번재설정/구글로그인 ④ 유료: 견적서 AI 자동입력(/api/parse)·엑셀·개인별 지출 정산
